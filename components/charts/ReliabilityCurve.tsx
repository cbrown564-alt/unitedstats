      ...
      >
        ...
        {grid.map((g) => (
          <g key={g}>
            ...
            <text x={x(g)} y={y(0) + 12} textAnchor="middle" fontSize={8} fill="var(--color-ink-faint)" className="stat-num">
              {g * 100}
            </text>
            <text x={x(0) - 6} y={y(g) + 3} textAnchor="end" fontSize={8} fill="var(--color-ink-faint)" className="stat-num">
              {g * 100}
            </text>
          </g>
        ))}
        ...
        {/* axis titles */}
        <text x={padL + plot / 2} y={H - 4} textAnchor="middle" fontSize={8} fill="var(--color-ink-faint)">
          Win expectancy (%)
        </text>
        <text
          x={12} y={padT + plot / 2} textAnchor="middle" fontSize={8} fill="var(--color-ink-faint)"
