 ...
 *
 * Pages that use this must also set `export const dynamicParams = false` (a Next
 * route-segment literal — it cannot be computed). Preview builds prerender only a
 * sample, so non-sampled ids 404; full builds prerender every id, so only
 * genuinely missing ids fall through to notFound().
 */
