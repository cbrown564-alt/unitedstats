...
async function fetchWikitext(season: string): Promise<string | null> {
  const cacheFile = path.join(CACHE, `${season}.wikitext`);
  if (fs.existsSync(cacheFile)) return fs.readFileSync(cacheFile, "utf8");
  for (const title of titleCandidates(season)) {
    const url =
...
function linkTarget(s: string): string | null {
  for (const m of s.matchAll(/\[\[([^[\]|#]+)(?:#[^[\]|]*)?(?:\|[^[\]]*)?\]\]/g)) {
    const target = m[1].trim();
    if (/^(penalty|own goal|cap \(|file:|image:)/i.test(target)) continue;
